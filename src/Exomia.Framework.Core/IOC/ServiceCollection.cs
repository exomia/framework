...
namespace Exomia.Framework.Core.IOC
{
    ...
    {
        private readonly Dictionary<Type, (Type, ServiceKind)>                           _typeEntries    = new Dictionary<Type, (Type, ServiceKind)>(16);
        private readonly Dictionary<Type, (Func<IServiceProvider, object>, ServiceKind)> _factoryEntries = new Dictionary<Type, (Func<IServiceProvider, object>, ServiceKind)>(16);
        ...
        public IServiceProvider Build(IServiceProvider? serviceProvider = null)
        {
            try
            {
                return ServiceProvider.Create(serviceProvider, _typeEntries, _factoryEntries);
            }
            finally
            {
                _typeEntries.Clear();
