...
public sealed unsafe partial class Canvas
{
    private static void RenderPolygon(Item* item, Vertex* vertex)
    {
        ...
        for (int i = 1; i < item->PolygonType.VerticesCount - 1; i++)
        {
            ...
            DrawRect(vertex + (i << 2), current, in ip1, in ip2, item);
        ...
        DrawRect(vertex,                                                current,  in ip1, in ip3, item);
        DrawRect(vertex + ((item->PolygonType.VerticesCount - 1) << 2), previous, in ip3, in ipE, item);

        static void DrawRect(Vertex* vertex, in Line2 lineA, in Vector2 bl, in Vector2 br, Item* item)
        {
            // p1
            vertex->XY    = lineA.XY1;
